...
{SPR_STAT_43},					// junk            "
#ifndef SPEAR
{SPR_STAT_44,bo_healthpack},	// pots            "
#else
{SPR_STAT_44,block},			// Gibs!
#endif
{SPR_STAT_45,bo_bandolier},		// bandolier           " (SOD:gibs)
...
#else
{SPR_BLINKY_W1,bo_25clip},		// ammobox (reusing sprites)
{436,bo_rocket},				// rocket launcher (explosions)
{SPR_PINKY_W2,bo_rocketammo},	// rocket ammo box (well ya need ammo for the rocket launcher)
{SPR_CLYDE_W2,bo_fuel},			// fuel can (slurp)
{SPR_INKY_W2,bo_flamethrower},	// flamethrower (cookin' with fire)
{SPR_STOVE_1,block},			// stove (the fact that i didn't use this sprite was really annoying me)
{SPR_DEMO,block},				// the open truck
{SPR_DEATHCAM,bo_godorb},		// godmode9
{SPR_BROWNPILL,block},			// brown pillar (from Spear of Destiny)
{SPR_CLYDE_W2,bo_usedfuel},		// used fuel can (my new "flammenwerfer ss" enemies will drop these upon death)
#endif
...
//
	for (type=0 ;  ; type++)
	{
		if (statinfo[type].picnum == -1)		// end of list
			Quit ("Could not find specified item type! Error 13");
...
// ohh macintoshdonald had a farm eieio
#ifndef PUSHWALL_TILE_TICS
#define PUSHWALL_TILE_TICS 64
#endif
...
void MovePWalls (void)
{
	...
	oldblock = pwallstate / PUSHWALL_TILE_TICS;

	pwallstate += tics;

	if (pwallstate / PUSHWALL_TILE_TICS != oldblock)
	{
	// block crossed into a new block
		...
		*(mapsegs[0]+farmapylookup[pwally]+pwallx) = player->areanumber+AREATILE;
		...
		//
		if (pwallstate >= PUSHWALL_TILE_TICS * 2) // move the two tiles while also fixing id software's typo
		{
		//
		// the block has been pushed two tiles
		//
			SD_StopSound();
			SD_PlaySound(PUSHSTOPSND);
			pwallstate = 0;
			return;
		}
		else
	...
	{
		// render the tile's position
		int tileprogress = pwallstate % PUSHWALL_TILE_TICS;
		int pos = (int)((long)tileprogress * 64 / PUSHWALL_TILE_TICS);
		if (pos > 63)
			pos = 63;
		pwallpos = (unsigned)pos;
	}
