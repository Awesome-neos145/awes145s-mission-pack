...
PML_OpenPageFile(void)
{
	...
	if (mmerror || !PMSegPages)
		Quit("PML_OpenPageFile: Out of memory for page list");
	...
	MM_GetPtr(&buf,size);
	if (mmerror || !buf)
		Quit("PML_OpenPageFile: Out of memory for offsets");
	...
	MM_GetPtr(&buf,size);
	if (mmerror || !buf)
		Quit("PML_OpenPageFile: Out of memory for lengths");
	if (!CA_FarRead(PageFile,(byte far *)buf,size))
