...
= T_FireProjectile
=
= not only is this used for player flames, but it also fixes the issue of the fireballs being framerate dependent
=================
*/

void T_FireProjectile (objtype *ob)
{
	long	deltax,deltay;
	int		damage;
	long	speed;
	...
	// look if the balls hit an enemy
	if (ob->obclass == fireobj && ob->temp1 == 1)
	{
		for (check=player->next; check; check=check->next)
		{
			if (check->flags & FL_SHOOTABLE)
			{
				...
				if (deltax < 0x6000l && deltay < 0x6000l)
				{
					// found and took the enemies lives
					DamageActor (check, (US_RndT() >> 2) + 40);	// damager, about 40-95
					PlaySoundLocActor(MISSILEHITSND,ob);
					ob->state = NULL;		// make em dissolve on impact
					return;
				}
			}
		}
...
void T_Projectile (objtype *ob)
{
	...
	// if you looked at the older code then you'll know what this one does
	if ((ob->obclass == rocketobj || ob->obclass == fireobj) && ob->temp1 == 1)
	{
		for (check=player->next; check; check=check->next)
		{
			if (check->flags & FL_SHOOTABLE)
			{
				...
				if (deltax < 0x6000l && deltay < 0x6000l)
				{
					// what
					if (ob->obclass == rocketobj)
						DamageActor (check, (US_RndT() >> 2) + 400);
					else
						DamageActor (check, (US_RndT() >> 2) + 40);	// 40-95
					PlaySoundLocActor(MISSILEHITSND,ob);
					if (ob->obclass == rocketobj)
						ob->state = &s_boom1;
					else
...
===============
*/
// bosses usually have a deadset direction, which means that they won't notice you if you're not in front of them
// nodir means they will notice you from all dirs
// i kept this the same for SpawnGift because the user must walk through an armor room
// Mecha 2-NZI is a possessed armor
...
void SpawnBoss (int tilex, int tiley)
{
	...
	new->dir = nodir; // explained at the top of this function
	...
	case guardobj:
		{
		 ...
		 #ifndef UPLOAD
		 PlaySoundLocActor(sounds[US_RndT()%9],ob);
		 #else
...
statetype s_fatdie6	= {false,SPR_FAT_DEAD,20,NULL,A_InstantVictory,&s_fatdie6};
...
statetype s_fire1 	= {false,SPR_FIRE1,6,T_FireProjectile,NULL,&s_fire2};
statetype s_fire2 	= {false,SPR_FIRE2,6,T_FireProjectile,NULL,&s_fire1};
...
statetype s_hitlerdie10	= {false,SPR_HITLER_DEAD,20,NULL,A_InstantVictory,&s_hitlerdie10};
