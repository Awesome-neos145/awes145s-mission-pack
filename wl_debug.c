...
//
	...
	if (mmerror)
		Quit ("Cannot take screenshot! Error 12");
	for (p=0;p<4;p++)
	{
...
//		gamestate.mapon++;
	...
	else if (Keyboard[sc_I])			// I = item cheat
	{
		...
		GivePoints (100000);
		HealSelf (99);
		gamestate.hasweapon[wp_pistol] = true; // arsqgtph
		gamestate.hasweapon[wp_machinegun] = true;
		gamestate.hasweapon[wp_chaingun] = true;
		gamestate.hasweapon[wp_flamethrower] = true;
		gamestate.hasweapon[wp_rocket] = true;
		gamestate.weapon = gamestate.chosenweapon = wp_rocket;
		gamestate.ammo = 199;
...
#endif
	// the K051649 and AY-3-8910 sound really good
	// a great example of this is in Metal Gear 2: Solid Snake
	else if (Keyboard[sc_Q])			// Q = infinite ammo
	{
		gamestate.infiniteammo ^= 1;
		CenterWindow (20,3);
	...
	else if (Keyboard[sc_S])			// S = slow motion
	{
		...
		CenterWindow (18,3);
		if (singlestep)
			US_PrintCentered ("Slow motion ON"); // why the hell would you want this
		else
