...
#define WARPNORTHTILE	104
#define WARPSOUTHTILE	106
#define TRUCKONETILE	82
#define TRUCKTWOTILE	84
#define TRUCKTHREETILE	86
#define TRUCKFOURTILE	88
#define SPEARTELETILE	2
...
//
		...
		SPR_ROCKETREADY=437,SPR_ROCKETATK1,SPR_ROCKETATK2,SPR_ROCKETATK3,
	SPR_ROCKETATK4,

	SPR_FLAMEREADY=442,SPR_FLAMEATK1,SPR_FLAMEATK2,SPR_FLAMEATK3,
	SPR_FLAMEATK4,
...
typedef struct objstruct
{
	...
	fixed 		x,y;
	byte		tilex,tiley;		// unsigned to byte
	byte		areanumber;
...
typedef	struct
{
	...
	boolean		hasweapon[NUMWEAPONS];	// tracks which weapons player owns
	...
	int			deathsequence;		// tracks special death sequence (0=none, 1-5=stages)
	long		deathsequencetime;	// timer for death sequence stages
	int			deathsequencecount;	// number of death sequence tiles
	boolean		deathsequencefaded;	// tracks if we've already faded to black
	struct { int tx, ty; } deathsequencetiles[100];	// array of death sequence tile coordinates
	boolean		infiniteammo;		// infinite ammo cheat mode
	char		message[40];
} gametype;
...
extern	boolean		singlestep,godmode,noclip,godflag;
...
extern  int			warpwx,warpwy,warpex,warpey,warpnx,warpny,warpsx,warpsy;			// warp def
extern	int			truckox,truckoy,trucktx,truckty,truckhx,truckhy,truckfx,truckfy;		// InstaWarp offsets
