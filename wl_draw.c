...
// 28 to make sure that my other walls don't use the same graphics for my doors
#define DOORWALL	(PMSpriteStart-28)
...
asm xor ch,ch // from Ripper on DieHard Wolfers, can cause whole display to fill with bg color
...
int	weaponscale[NUMWEAPONS] = {SPR_KNIFEREADY,SPR_PISTOLREADY
	,SPR_MACHINEGUNREADY,SPR_CHAINREADY,SPR_FLAMEREADY,SPR_ROCKETREADY};
...
void DrawMessage (void)
{
	...
	if (messagetime > 0)
	{
		...
		// Draw shadow
		SETFONTCOLOR(0x0,0x0); // dark shadow color
		PrintX=9;
		PrintY=3; // shadow offset position
