...
#define TIMEY	8
	...
	#ifndef SPEAR
	if (Keyboard[sc_P] && MS_CheckParm("cheater"))
		PicturePause();
	#endif
...
void PreloadGraphics(void)
{
	...
	SETFONTCOLOR(0x0,0x0); // set up shadow
	PrintY=3;
	PrintX=1; 
	...
	SETFONTCOLOR(0xf,0x0); // set up showing map name
	PrintY=2; 
	fontnumber=0; // WSJ->Chris->KyleRTCW's disappearing text bug fix.
	US_CPrint("Entering:");
	while (mapheaderseg[gamestate.mapon+gamestate.episode*10]->name[pos]>=32 && pos<16)
	...
	mapname[pos] = 0;
	US_CPrint(mapname);
	// making use of the map name in GAMEMAPS, thank you Adam Biser
...
void NonShareware(void)
{
	...
	#else
	US_Print("For updates:\n");
	US_Print("https://awes145.com/amp\n");
	US_Print("\n");
	#endif
	US_Print("        -awes145\n");
