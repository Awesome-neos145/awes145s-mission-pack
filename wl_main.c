...
void NewGame (int difficulty,int episode)
{
	...
	gamestate.fuel = 0;
	gamestate.maxfuel = 99;
	gamestate.lives = 3;
	gamestate.nextextra = EXTRAPOINTS;
	gamestate.episode=episode;

	// new weapon ownership system!!!
...
boolean LoadTheGame(int file,int x,int y)
{
	...
	if (oldchecksum != checksum)
	{
	 ...
	 // ...soo i maybe should make this more brutal
	 // considering the fact we have debug mode and, regardless of that, the user still edited their save
	 // however since the game probably can't draw nothing there, i'll leave it
	 gamestate.hasweapon[wp_knife] = true;
	 gamestate.hasweapon[wp_pistol] = false;
	 gamestate.hasweapon[wp_machinegun] = false;
	 gamestate.hasweapon[wp_chaingun] = false;
...
static  int     wolfdigimap[] =
		{
			...
			MOVINGSND,				46,		// moving positions...
			PUSHSTOPSND,			47,		// pushwall stop sound
			FALLSND,				48,		// falling sound
			DRAGSND,				49,		// dragging sound
			HEILSND,				50,
            ACHTUNG,				51,
            KEVASND,				52,
            HALTENSIE,				53,
...
//
			...
			CA_CacheGrChunk (TITLEPIC2);
			VWB_DrawPic (0,80,TITLEPIC2);
			UNCACHEGRCHUNK (TITLEPIC2);
			VW_UpdateScreen ();
			VW_FadeIn();
#else
...
#ifndef SPEAR
		if (Keyboard[sc_Tab] && (MS_CheckParm("cheater") || Keyboard[sc_LShift]))
#else
		if (Keyboard[sc_Tab] && (MS_CheckParm("debugmode") || Keyboard[sc_LShift]))
#endif
		{
			RecordDemo ();
			StartCPMusic(INTROSONG);
