...
boolean		singlestep,godmode,noclip,godflag;
int			extravbls;
...
//
	...
	if ( (scan >= sc_F2 && scan <= sc_F9) || scan == sc_Escape)
	{
		StopMusic ();
		ClearMemory ();
		VW_FadeOut ();
...
void RemoveObj (objtype *gone)
{
	...
	if (gone == player)
		Quit ("Player somehow evaporated from existence! Error 3");
...
void HandleDeathSequence (void)
{
	static byte blackpal[768];
	
	if (!gamestate.deathsequence)
		return;

	gamestate.deathsequencetime += tics;

	// black palette
	if (!gamestate.deathsequencefaded)
	{
		memset(blackpal, 0, 768);
		VL_SetPalette(blackpal);
		gamestate.deathsequencefaded = true;
	}
	else
	{
		// make sure it stays black
		// some video encoders while recording (v4l2) could capture the normal screen, but this is a little rare...
		VL_SetPalette(blackpal);
	}

	switch (gamestate.deathsequence)
	{
	case 1:
		// stop the music
		if (gamestate.deathsequencetime <= tics)
		{
			StopMusic ();
			SD_PlaySound (FALLSND);	// fall
		}
		// waiting
		if (gamestate.deathsequencetime > 75)
		{
			gamestate.deathsequence = 2;
			gamestate.deathsequencetime = 0;
		}
		break;
	...
	case 2:
		...
		if (gamestate.deathsequencetime <= tics)
		{
			SD_PlaySound (AHHHGSND);	// collapse
		}
		if (gamestate.deathsequencetime > 500)
		{
			// take stuff away for immerion when they see the statusbar in the victory sequence
			gamestate.hasweapon[wp_pistol] = false;
			gamestate.hasweapon[wp_machinegun] = false;
			gamestate.hasweapon[wp_chaingun] = false;
			gamestate.hasweapon[wp_flamethrower] = false;
			gamestate.hasweapon[wp_rocket] = false;
			gamestate.ammo = 0;
			gamestate.rockets = 0;
			gamestate.fuel = 0;
			gamestate.weapon = gamestate.chosenweapon = wp_knife;
			SD_PlaySound (DRAGSND);	// bj getting dragged to prison
			gamestate.deathsequence = 3;
			gamestate.deathsequencetime = 0;
		}
		break;

	case 3:
		// i'm waiiitttiiiingggg
		if (gamestate.deathsequencetime > 300)
		{
			// and that's a wrap
			playstate = ex_victorious;
			gamestate.deathsequence = 0;
			DrawHealth();
			DrawAmmo();
			DrawWeapon();
			DrawFace();
...
void PlayLoop (void)
{
	...
	do
	{
		...
		// call sequence if tile is walked over
		if (gamestate.deathsequence)
		{
			HandleDeathSequence ();
		}
		else
		{
			...
			{
				int i;
				for (i = 0; i < gamestate.deathsequencecount; i++)
				{
					if (player->tilex == gamestate.deathsequencetiles[i].tx && 
						player->tiley == gamestate.deathsequencetiles[i].ty)
					{
						gamestate.deathsequence = 1;
						gamestate.deathsequencetime = 0;
						gamestate.deathsequencefaded = false;
						break;
					}
				}
			}
...
//
		if (!gamestate.deathsequence)
		{
			...
			// threedrefresh if not in sequence
			ThreeDRefresh ();
		}
		...
		heartbeatcount += tics;
		if (heartbeatcount > 8*16 && gamestate.health <= 20)
		{
			heartbeatcount = 0;
