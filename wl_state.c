...
boolean TryWalk (objtype *ob)
{
	...
	if (ob->obclass == inertobj)
	{
		...
		default:
			Quit ("Enemy was not meant to move in such direction! Error 2");
		}
...
void KillActor (objtype *ob)
{
	...
	case ssobj:
		...
		if (!gamestate.hasweapon[wp_machinegun])
	...
	case gretelobj:
		...
		if (!(gamestate.keys & 1))
			...
			PlaceItemType (bo_25clip,tilex,tiley);
		break;
	...
	case fakeobj:
		GivePoints (800);
		PlaceItemType (bo_usedfuel,tilex,tiley);
...
{
	if (ob->flags & FL_ATTACKMODE)
		Quit ("An actor saw the player again and got confused! Error 8");
